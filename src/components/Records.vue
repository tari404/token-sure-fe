  ...
  <div class="records">
    <ul>
      <li
        ...
        :key="i"
        :class="`record-item record-item-${item.dir}`"
        :finished="item.finished"
      >
        <p>
          {{ item.dir === 'in' ? '承接保单' : '发布保单' }}{{ item.finished ? ' - 已完成' : '' }}
        </p>
        <span class="token">{{ item.amount }} {{ item.token }}</span>
        <span class="apy"><span style="color: #606266;">APY: </span>{{ item.apy }}%</span>
        <el-button v-if="!item.finished" @click="toRemove(item)" type="text" size="small">
          清算
        </el-button>
        <el-button v-else @click="toEnd(item)" type="text" size="small">
          结束保单
        </el-button>
      </li>
    </ul>
...
.record-item-in
  ...
  &[finished]
    background-image linear-gradient(to right, rgba(#ea0070, 0) 40%, rgba(#ea0070, 0.07))
.record-item-out
  border-left-color #74a700
  &[finished]
    background-image linear-gradient(to right, rgba(#74a700, 0) 40%, rgba(#74a700, 0.1))
